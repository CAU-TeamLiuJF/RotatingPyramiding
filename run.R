...
# 一些文件的保存路径
tmp_dir <- file.path(work_dir, 'tmp', paste0('G', targetGeneNum))
...
# 一些文件的保存路径
output_dir <- file.path(work_dir, 'output', paste0('G', targetGeneNum))
...
for (repeat_idx in 1:repeat_num){
  ...
  tmp_result_file <- paste0('result_Mao_', scenario_id, '_repeat_', repeat_idx, '.RData')
  # 断点续跑
  # if (file.exists(tmp_result_file)){
  #   next
  # }

  print(paste0('目标基因数 ', targetGeneNum, ' 情景 ', scenario_id, ' 第 ', repeat_idx, ' 次重复...'))
  ...
  # 从第 6 代开始各种情况
  scenario_dir <- file.path(work_dir, strategy_name, paste0('Mao_', scenario_id, '.R'))
  print(scenario_dir)
  source(scenario_dir)

  save(data_list, file=file.path(tmp_dir, tmp_result_file))
}
